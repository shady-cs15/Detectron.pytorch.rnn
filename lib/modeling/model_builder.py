        ...
        for t in range(time_steps):
            ...
            if self.training:
                ...
                for k, v in rpn_kwargs.items():
                    cur_kwargs[k] = rpn_kwargs[k][t]
                cur_kwargs.update(dict(
                    (k, rpn_ret[k]) for k in rpn_ret.keys()
                    if (k.startswith('rpn_cls_logits') or k.startswith('rpn_bbox_pred'))
                ))
                loss_rpn_cls, loss_rpn_bbox = rpn_heads.generic_rpn_losses(**cur_kwargs)
                if cfg.FPN.FPN_ON:
                    for i, lvl in enumerate(range(cfg.FPN.RPN_MIN_LEVEL, cfg.FPN.RPN_MAX_LEVEL + 1)):
                        if 'loss_rpn_cls_fpn%d' % lvl not in return_dict['losses']:
                            return_dict['losses']['loss_rpn_cls_fpn%d' % lvl] = loss_rpn_cls[i]
                        else:
                            return_dict['losses']['loss_rpn_cls_fpn%d' % lvl] += loss_rpn_cls[i]

                        if 'loss_rpn_bbox_fpn%d' % lvl not in return_dict['losses']:
                            return_dict['losses']['loss_rpn_bbox_fpn%d' % lvl] = loss_rpn_bbox[i]
                        else:
                            return_dict['losses']['loss_rpn_bbox_fpn%d' % lvl] += loss_rpn_bbox[i]       
                else:
                    if 'loss_rpn_cls' not in return_dict['losses']:
                        return_dict['losses']['loss_rpn_cls'] = loss_rpn_cls
                    else:
                        return_dict['losses']['loss_rpn_cls'] += loss_rpn_cls
                    if 'loss_rpn_bbox' not in return_dict['losses']:
                        return_dict['losses']['loss_rpn_bbox'] = loss_rpn_bbox
                    else:
                        return_dict['losses']['loss_rpn_bbox'] += loss_rpn_bbox
                ...
                loss_cls, loss_bbox, accuracy_cls = fast_rcnn_heads.fast_rcnn_losses(
                    cls_score, bbox_pred, rpn_ret['labels_int32'], rpn_ret['bbox_targets'],
                    rpn_ret['bbox_inside_weights'], rpn_ret['bbox_outside_weights'])
                if 'loss_cls' not in return_dict['losses']:
                    return_dict['losses']['loss_cls'] = loss_cls
                else:
                    return_dict['losses']['loss_cls'] += loss_cls
                if 'loss_bbox' not in return_dict['losses']:
                    return_dict['losses']['loss_bbox'] = loss_bbox
                else:
                    return_dict['losses']['loss_bbox'] += loss_bbox
                if 'accuracy_cls' not in return_dict['metrics']:
                    return_dict['metrics']['accuracy_cls'] = accuracy_cls/time_steps
                else:
                    return_dict['metrics']['accuracy_cls'] += accuracy_cls/time_steps
